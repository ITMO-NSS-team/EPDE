    ...
    for magnitude in magnitudes:
        ...
        for idx in range(test_launches):
            if run_epde:
                ...
                try:
                    logger.add_log(key = f'Lotka_Volterra_noise_{magnitude}_attempt_{idx}', entry = sys, aggregation_key = ('epde', magnitude),
        ...
        if run_sindy:            
            ...
            for idx in range(test_launches):             
                ...
                try:
                    logger.add_log(key = f'Lotka_Volterra_SINDy_noise_{magnitude}_attempt_{idx}', entry = sys, aggregation_key = ('sindy', magnitude), 
                                   error_pred = (err_u, err_v))
                except NameError:
