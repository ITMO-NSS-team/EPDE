...
if __name__ == '__main__':
    ...
    exps = {}
    test_launches = 5
    magnitudes = [1.5 * 1e-1, 2. * 1e-1, 2.5 * 1e-1] # [1.*1e-2, 2.5*1e-2, 5.*1e-2, 1.*1e-1]
    for magnitude in magnitudes:
        ...
        if run_epde:
            for idx in range(test_launches):
                ...
                try:
                    logger.add_log(key = f'Burgers_{magnitude}_attempt_{idx}', entry = sys, error_pred = np.mean(np.abs(data_test - pred_u_v)))
                except NameError:
        ...
        if run_sindy:
            ...
            calc_sindy = pred_u_v
            try:
                logger.add_log(key = f'Burgers_sindy_{magnitude}', entry = sys, error_pred = np.mean(np.abs(data_test - pred_u_v)))
            except NameError:
