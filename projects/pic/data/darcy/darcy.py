...
def darcy_test(operator: CompoundOperator, foldername: str, noise_level: int = 0):
    ...
    _, domain = epde_search_obj.createDomain(grid, boundary_width=0, ID=0)
    ...
    epde_search_obj.create_pool(data=[trajectory], max_deriv_order=(0, 2, 2),
...
def darcy_discovery(foldername, noise_level):
    ...
    bounds = (1e-12, 1e-2)
    _, trajectory = epde_search_obj.createTrajectory({'u': noised_data}, domain, cache_id=0)
    epde_search_obj.fit(data=[trajectory], max_deriv_order=(0, 2, 2), data_fun_pow=1,
