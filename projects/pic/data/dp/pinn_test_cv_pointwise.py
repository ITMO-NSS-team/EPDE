"""
CV-of-residual-variance DP PINN -- two candidate equations.

Same physics term as the baseline (TRUE coefs in the residual), but the CV
penalty is computed in PREDICTION space rather than in theta space:

    L_phys = sum_eq mean( (target_eq - features_eq @ theta_true_eq)^2 )
    L_cv   = sum_eq mean_k( var_{i in W_k}( y_k - features_k @ theta_i ) )
    L_data = mean( (theta1_pred - theta1_data)^2 + (theta2_pred - theta2_data)^2 )
    L      = W_PHYS * L_phys + W_CV * L_cv + W_DATA * L_data

For each collocation point t_k, gather the residuals produced at t_k by every
window's OLS theta_i (only windows that cover t_k contribute -- the existing
sliding-window mask). Population variance across those windows measures
inter-window disagreement at the point: small if all windows' OLS solutions
agree on the residual structure, large if they disagree.

Compare against `pinn_test_cv.py` which measures CV directly in theta space
via anchored_mse on per-window OLS theta. Both share the same physics term
and the same OLS gradient path (`torch.linalg.solve`); only the CV form
differs.

Physics + CV + data losses are computed on the TRAIN slice of the data grid
(t in [0, TRAIN_FRAC * T_MAX]) each iteration. Windows still span the full
[0, T], but the OLS mask in the CV penalty is built from TRAIN-ONLY points
-- the held-out tail never enters the loss. Post-hoc OLS at evaluation runs
on the full grid so per-window coefficients can be inspected across both
...
CIRCULAR_WINDOWS = True  # True -> treat t-axis as a loop (windows wrap at T_MAX)
EPS = 1e-8
...
def physics_and_cv_loss(net, T_coll):
    ...
    CV term is the per-point variance of per-window OLS residuals (averaged
    over points), summed across the two candidate equations. Physics residual
    is normalized by max(|y|) per equation so the gravity-dominated absolute
    scale doesn't drown the smaller coupling-coef contributions.
    """
    ...
    for eq in EQUATIONS:
        ...
        # Per-point variance of per-window residuals (gradient through linalg.solve).
        l_cv_total = l_cv_total + per_point_residual_variance(
            theta_pw, y, A, mask, eps=EPS)
...
# L-BFGS on the same train-grid pool
...
print(f"\n========== DP CV-POINTWISE PINN (per-point residual variance) ==========")
...
out_path = f"dp_pinn_cv_pointwise{'_circular' if CIRCULAR_WINDOWS else ''}.npz"
