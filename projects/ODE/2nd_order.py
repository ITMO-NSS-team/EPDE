...
def epde_discovery(t, x, use_ann = False):
    ...
    trig_tokens = epde.TrigonometricTokens(freq = (2 - 0.0000005, 2 + 0.0000005), 
...
if __name__ == "__main__":
    ...
    try:
        data_file =  os.path.join(os.path.dirname( __file__ ), 'projects/ODE/data/ODE_sol.npy')
        data = np.load(data_file)
    except FileNotFoundError:
        data_file = '/home/maslyaev/epde/EPDE_main/projects/ODE/data/ODE_sol.npy'
        data = np.load(data_file)
    ...
    if use_epde:
        epde_search_obj, sys = epde_discovery(t_train, x_train, False)
