...
def epde_discovery(t, x, angle, u, derivs, diff_method = 'FD', data_nn: torch.nn.Sequential = None, 
                   device: str = 'cpu', ctrl_net: torch.nn.Sequential = None):
...
def general(experiment, res_folder, single_sample: bool = True, discover: bool = True, 
            device: str = 'cpu'): # fourier_params: dict = {'L' : [4,], 'M' : [3,]}
    env_config = {'domain_name': "cartpole",
                  'task_name': "swingup",
                  'frame_skip': 1,
                  'from_pixels': False}
...
if __name__ == '__main__':
    ...
    if only_prepare:
        ...
        t, ctrl, solution, res = general(experiment, res_folder=res_folder, discover=discover, 
                                         device = device)
