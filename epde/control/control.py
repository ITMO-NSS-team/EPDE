...
class DeepXDEAdapter:
    """
    SolverAdapter ===> DeepXDEAdapter
    """

    def __init__(self, net: torch.nn.Sequential = None, use_cache: bool = False, device: str = "cpu"):
        self.net = net
    ...
    def set_compiling_params(self, **kwargs):
        self._compiling_params.update(kwargs)

    def set_optimizer_params(self, **kwargs):
        self._optimizer_params.update(kwargs)

    def set_cache_params(self, **kwargs):
        self._cache_params.update(kwargs)

    def set_early_stopping_params(self, **kwargs):
        self._early_stopping_params.update(kwargs)

    def set_plotting_params(self, **kwargs):
        self._plotting_params.update(kwargs)

    def set_training_params(self, **kwargs):
        self._training_params.update(kwargs)

    def change_parameter(self, key, value, param_dict_key: str = "compiling_params"):
        if param_dict_key == "compiling_params":
            self._compiling_params[key] = value
            if key == "mode":
                self._mode = value
        elif param_dict_key == "optimizer_params":
            self._optimizer_params[key] = value
        elif param_dict_key == "cache_params":
            self._cache_params[key] = value
        elif param_dict_key == "early_stopping_params":
            self._early_stopping_params[key] = value
        elif param_dict_key == "plotting_params":
            self._plotting_params[key] = value
        elif param_dict_key == "training_params":
            self._training_params[key] = value

    def set_net(self, net: torch.nn.Sequential):
        self.net = net

    def _build_dde_problem(self, system, grids, boundary_conditions, use_fourier, fourier_params, use_adaptive_lambdas):
        """
        DeepXDE model
        """
        # domain, pde, bcs, ic, data
        # model = dde.Model(data, net_dde)
        # return model
        raise NotImplementedError("DeepXDE task realization")

    def solve_epde_system(
        self,
        system,
        grids,
        data,
        ...
        use_fourier,
        fourier_params,
        use_adaptive_lambdas,
    ):
        # Build DeepXDE task
        model = self._build_dde_problem(
            system=system,
            grids=grids,
        ...
        optimizer_name = self._optimizer_params.get("optimizer", "adam")
        lr = self._optimizer_params.get("lr", 1e-3)
        epochs = self._training_params.get("epochs", 150)

        if optimizer_name.lower() == "adam":
            model.compile("adam", lr=lr)
        else:
            model.compile(optimizer_name, lr=lr)

        model.train(epochs=epochs)

        loss_hist = model.losshistory.loss_train
        final_loss = float(loss_hist[-1]) if len(loss_hist) > 0 else 0.0
...
class ControlExp():
    ...
    def set_solver_params(self, use_pinn: bool = True, mode: str = 'autograd', compiling_params: dict = {},
                          ...
                          use_cache: bool = False, use_fourier: bool = False,
    ...
    def get_solver_adapter(self, net: torch.nn.Sequential = None):
        if self._use_pinn:
            adapter = DeepXDEAdapter(net=net, use_cache=False, device=self._device)
    ...
    def finite_diff_calculation(system, adapter, loc, control_loss, state_net: torch.nn.Sequential,
        ...
        if isinstance(adapter, DeepXDEAdapter):
    ...
    def feedback(self, bc_operators: List[Union[dict, float]], grids: List[Union[np.ndarray, torch.Tensor]],
        ...
        if isinstance(state_net, torch.nn.Sequential):
        ...
        if isinstance(grids[0], np.ndarray):
            grids_merged = torch.from_numpy(np.array([subgrid.reshape(-1) for subgrid in grids])).float().T.to(
                self._device)
        ...
        while t < epochs and not stop_training:
            ...
            if fig_folder is not None and LV_exp:
                ...
                plt.plot(grids_merged.cpu().detach().numpy(),
                         global_var.control_nn.net(control_inputs).cpu().detach().numpy(),
                         color='tab:orange')
                plt.grid()
