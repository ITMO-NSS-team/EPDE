...
class Term(ComplexStructure):
    ...
    def cache_label(self):
        if len(self.structure) > 1:
            ...
            cache_label = tuple(
                [elem.cache_label for elem in structure_sorted])  # reduce(form_label, structure_sorted, '')
        else:
    ...
    def _(self, passed_term: list):
        ...
        for _, factor in enumerate(passed_term):
            if isinstance(factor, str):
                ...
                raise ValueError(
                    'The structure of a term should be declared with str or factor.Factor obj, instead got',
                    type(factor))
    ...
    def _(self, passed_term: str):
        ...
        if isinstance(passed_term, str):
            ...
            raise ValueError('The structure of a term should be declared with str or factor.Factor obj, instead got',
                             type(passed_term))
            ...
            for family in self.pool.labels_overview:
                ...
                for token_label in family[0]:
                    if isinstance(self.max_factors_in_term, int):
                        ...
                        forbidden_factors[token_label] = [0,
                                                          min(self.max_factors_in_term['factors_num'][-1], family[1]),
                                                          False]
        ...
        if self.saved[structural] or (self.cache_label, normalize) in global_var.tensor_cache:  #
            ...
            if np.all([len(factor.params) == 1 for factor in self.structure]):
                self.saved[normalize] = global_var.tensor_cache.add(self.cache_label, value,
                                                                    normalized=normalize)  # Место возможных проблем: сохранение/загрузка нормализованных данных
    ...
    def __eq__(self, other):
        return (all([any([other_elem == self_elem for other_elem in other.structure]) for self_elem in self.structure])
                and all(
                    [any([other_elem == self_elem for self_elem in self.structure]) for other_elem in other.structure])
...
class StructureStatus1:
    def __init__(self, equation):
        self.check_forbid_hashes = None
        self.equation = equation

        token_ls = []
        for family in self.equation.pool.families:
            token_ls += family.tokens

        term_ls = []
        for i in range(1, self.equation.metaparameters['max_factors_in_term']['value'] + 1):
            term_ls += list(itertools.combinations(token_ls, i))

        ############# удалить позже #######################
        forbidden_terms_label_ls = []
        for term in self.equation.structure:
            forbidden_terms_label_ls.append(self.equation.compile_term(term))
        self.forbidden_terms_label = forbidden_terms_label_ls
        ###################################################

        term_ls_hashed = [self.hash_term(term) for term in term_ls]
        custom_prob_hashed = [self.hash_term(term) for term in self.equation.pool.custom_prob_terms.keys()]
        self.forbidden_terms_hashed = set([self.hash_term(term) for term in forbidden_terms_label_ls])

        # два полных словаря, неизменны для одного пула
        self.term_ls_dict = dict(zip(term_ls_hashed, term_ls))
        self.custom_prob_dict = dict(zip(custom_prob_hashed, self.equation.pool.custom_prob_terms.values()))

        term_set_hashed = set(term_ls_hashed)
        self.custom_prob_set_hashed = set(custom_prob_hashed)
        # два частичных сета
        self.available_hashes = term_set_hashed.difference(self.forbidden_terms_hashed)
        self.available_custom_hashes = self.custom_prob_set_hashed.difference(self.forbidden_terms_hashed)

    def check_eq_structure(self):

        structure_set = set()
        for term in self.equation.structure:
            label = self.equation.compile_term(term)
            structure_set.add(self.hash_term(label))

        if structure_set != self.forbidden_terms_hashed:
            return 'UNDETECTED TERMS FOUND'
        else:
            return "status checked"
    def update_forbidden_terms(self):
        forbidden_terms_label_ls = []
        for term in self.equation.structure:
            forbidden_terms_label_ls.append(self.equation.compile_term(term))
        self.forbidden_terms_label = forbidden_terms_label_ls
        
        self.check_forbid_hashes = []
        for hashed in self.forbidden_terms_hashed:
            self.check_forbid_hashes.append(self.term_ls_dict.get(hashed))
            
    def update(self, old_term, new_term):
        new_label = self.equation.compile_term(new_term)
        old_label = self.equation.compile_term(old_term)

        new_hash = self.hash_term(new_label)
        old_hash = self.hash_term(old_label)

        self.available_hashes.add(old_hash)
        self.available_hashes.remove(new_hash)
        self.forbidden_terms_hashed.add(new_hash)
        self.forbidden_terms_hashed.remove(old_hash)

        self.available_custom_hashes.discard(new_hash)
        if old_hash in self.custom_prob_set_hashed:
            self.available_custom_hashes.add(old_hash)
    @staticmethod
    def hash_term(term):
        total_term = 0
        for token in term:
    ...
    def define_mutation_prob(self):
        ...
        units_quantity = len(self.available_hashes) - len(self.available_custom_hashes)
        correction_units = 0.

        for hash_key in self.available_custom_hashes:
            correction_units += self.custom_prob_dict.get(hash_key)
        units_quantity += correction_units

        probability_unit = 1. / units_quantity

        '''формируем список вероятностей'''
        prob_ls, term_ls = [], []
        for available_hash in self.available_hashes:
            term_ls.append(available_hash)
            if available_hash in self.available_custom_hashes:
                prob_ls.append(self.custom_prob_dict.get(available_hash) * probability_unit)
            else:
                prob_ls.append(probability_unit)
...
class StructureStatus:
    ...
    def hash_forbid_terms(self):
        ...
        for term in self.equation.structure:
            label = self.equation.compile_term(term)
            forbidden_terms_hashed.add(self.hash_term(label))
    ...
    def define_mutation_prob(self):
        ...
        for available_hash in available_hashes:
            ...
            if available_hash in available_custom_hashes:
                prob_ls.append(self.equation.pool.prob_info.custom_prob_dict.get(available_hash) * probability_unit)
            else:
                prob_ls.append(probability_unit)
        return term_ls, prob_ls
...
class Equation(ComplexStructure):
    ...
    def compile_term(term):
        complete_term = []
        for i in range(len(term.cache_label)):
            if type(term.cache_label[1][0]) == np.float64:  # if type(term.cache_label[0]) == np.str_
                complete_term.append(term.cache_label[0])
                break
            else:
                complete_term.append(term.cache_label[i][0])
    ...
    def __eq__(self, other):
        if self.weights_final_evald and other.weights_final_evald:
            return (all([any([other_elem == self_elem for other_elem in other.structure]) for self_elem in
                         self.structure])
                    and all([any([other_elem == self_elem for self_elem in self.structure]) for other_elem in
                             other.structure])
                    and len(other.structure) == len(self.structure)
            ...
            if normalize:
                ...
                value = np.add(elem1,
                               - reduce(lambda x, y: np.add(x, y), [np.multiply(weight, temp_feats[:, feature_idx])
                                                                    ...
                                                                    np.ndenumerate(self.weights_internal)]))
            else:
                elem1 = np.expand_dims(self._target, axis=1)
        ...
        if self.solver_form_defined:
            ...
            for term_idx in range(len(self.structure)):
                if term_idx != self.target_idx:
                    ...
                    weight = self.weights_final[term_idx] if term_idx < self.target_idx else self.weights_final[
                        term_idx - 1]
    ...
    def boundary_conditions(self, main_var_key=('u', (1.0,)), full_domain: bool = False):
        ...
        for ax_idx, ax_ord in enumerate(required_bc_ord):
            for loc_fraction in hardcoded_bc_relative_locations[ax_ord]:
                ...
                vals = np.expand_dims(tensor_cache.get(main_var_key)[indexes],
                                      axis=0).T  # np.squeeze(tensor_cache.get(main_var_key)[indexes]).T
...
class SoEq(moeadd.MOEADDSolution):
    def __init__(self, pool: TF_Pool, metaparameters: dict):
        ...
        self.tokens_supp = TF_Pool(pool.families_supplementary, custom_prob_terms=pool.custom_prob_terms,
                            custom_cross_prob=pool.custom_cross_prob, max_factors_in_term=pool.max_factors_in_term)
    ...
    def use_default_objective_function(self):
        ...
        quality_objectives = [generate_partial(equation_fitness, eq_key) for eq_key in
                              self.vars_to_describe]  # range(len(self.tokens_for_eq))]
    ...
    @staticmethod
    def equation_opt_iteration(population, evol_operator, population_size, iter_index, unexplained_vars,
