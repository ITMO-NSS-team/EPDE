...
class L2Fitness(CompoundOperator):
    ...
    def apply(self, objective: Equation, arguments: dict):
        ...
        self.suboperators['coeff_calc'].apply(
            objective, subop_args['coeff_calc'])

        _, target, features = objective.evaluate(
            normalize=False, return_val=False)
        try:
        ...
        if not (self.params['penalty_coeff'] > 0. and self.params['penalty_coeff'] < 1.):
            raise ValueError(
                'Incorrect penalty coefficient set, value shall be in (0, 1).')
    ...
    def use_default_tags(self):
        self._tags = {'fitness evaluation', 'gene level',
