...
class PhysicsInformedLasso(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y, sample_key=None, grid_shape=None, sample_weights=None, gram_setup=None): #  
        if sample_weights is not None:
            grid_shape = sample_weights.shape
...
class VWSRSparsity(CompoundOperator):
    ...
    def apply(self, objective : Equation, arguments : dict):
        ...
        assert isinstance(target, dict) and isinstance(features, dict), \
            f'Unexpected behavior: target and features, obtained from obj.evaluate have to be dicts, \
              instead got: {type(target)} and {type(features)}.'
        
        gfuncs = global_var.samples_manager.gFunc('dm')
        # fit = lambda *args: estimator.fit(args[0], args[1], args[2], gram_setup=gram_setup)
        # estim_func = partial()

        sampled_weights_internal = []
        sampled_intercepts = []

        for key in target.keys():
            estimator.fit(features[key], target[key], sample_weights=gfuncs[key], gram_setup=gram_setup) # , sample_key = key,
            sampled_weights_internal.append(estimator.coef_)
            sampled_intercepts.append(estimator.intercept_)

        sampled_weights_internal = np.stack(sampled_weights_internal, axis = 1)
        sampled_weights_internal = np.mean(sampled_weights_internal, axis = 1)
        sampled_intercepts = np.mean(sampled_intercepts)

        objective.weights_internal = sampled_weights_internal
        objective.weights_internal_evald = True
        objective.weights_final = np.append([weight for weight in objective.weights_internal if weight != 0], sampled_intercepts)
        objective.weights_final_evald = True
