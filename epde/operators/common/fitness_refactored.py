...
class L2Fitness(CompoundOperator):
    ...
    def apply(self, objective: Equation, arguments: dict, force_out_of_place: bool = False):
        ...
        _, targets, features = objective.evaluate(normalize = False, return_val = False)
        if features is None:
            discr_feats = 0
        else:
            ...
            if n_cols == len(mask):
                discr_feats = dictApplyUFunc(np.dot, features, objective.weights_internal)
            elif n_cols == int(mask.sum()):
                discr_feats = dictApplyUFunc(np.dot, features, objective.weights_final[:-1])
            else:
                discr_feats = dictZerosLike(features, 0)

        # discr = dictSubtr(dictAdd(discr_feats, dictFullLike(targets.shape, objective.weights_final[-1])), targets)
        discr = dictSubtr(dictAdd(discr_feats, dictFullLike(targets.shape, objective.weights_final[-1])), targets)
        
        try:
            self.g_fun_vals = global_var.samples_manager.gFunc(mode_key = 'f')
            # global_var.grid_cache.g_func[global_var.grid_cache.g_func_mask].reshape(-1)
        except AttributeError:
            self.g_fun_vals = None
        if self.g_fun_vals is not None and self.g_fun_vals.shape == discr.shape:
            discr = dictApplyUFunc(np.multiply, discr, self.g_fun_vals) # np.multiply(discr, self.g_fun_vals)
        
        l2norm = partial(np.linalg.norm, ord=2)
        rl_error = dictApplyUFunc(l2norm, discr)
        ...
        try:
            ...
            if sw_features is None:
                ...
                if hasattr(objective, '_cached_sw_weights') and objective._cached_sw_weights is not None:
                    ...
                    cw_func = partial(calculate_weights, fit_intercept=objective.weights_final[-1] != 0)
                    sw_weights = dictApplyUFunc(cw_func, sw_features, sw_target, self.g_fun_vals, data_shapes)

                    # sw_weights = calculate_weights(
                    #     sw_features, sw_target, self.g_fun_vals, data_shape,
                    #     objective.weights_final[-1] != 0,
                    # )
                for key, sw_arr in sw_weights:
                    sw_arr = np.array(sw_arr) # TODO: check, if it is redundant
                    std = sw_arr.std(axis=0, ddof=1)
                    mu = sw_arr.mean(axis=0)
                    with np.errstate(divide='ignore', invalid='ignore'):
                        cv = (std ** 2) / (mu ** 2)
                    total_lr = sum(cv) / len(data_shapes[key])
        except Exception:
            total_lr = 1.0
