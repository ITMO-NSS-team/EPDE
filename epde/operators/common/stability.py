...
def resolve_vc_modes_from_input(grid_shape, main_var=None, sample_key: int = 0,
    ...
    for var in var_names:
        try:
            # raise RuntimeError('FIX FURTHER CODE!!!')
            temp = _gv.samples_manager.get((var, (1.0,)), 0) # , sample_key=sample_key
            field = temp[sample_key]
        except Exception:
            continue
        for d in range(D):
            lam = taylor_microscale(field, grid_shape, d) # , sample_key=sample_key
            if lam < lam_min[d]:
                lam_min[d] = lam
...
class VaryingCoefSetup:
    ...
    *non-constant* energy is small relative to its constant part; the
    ...
    def __init__(self, X, y, sample_weights, grid_shape, main_var: str = None, sample_key=None,
        ...
        # resolve_partial = partialmethod(self._resolve_modes, modes=modes, main_var=main_var, k_max=k_max)
        modes = self._resolve_modes(modes, grid_shape, main_var, k_max, sample_key = sample_key)
