...
def parse_parametric_factor(factor_form : str, pool):
    ...
    params_str, params_to_optimize = parse_params_str(params_str.replace('}', ''))
    
    # print(label_str, params_str)
    contains_parametric = len(params_to_optimize) > 0
    return contains_parametric, label_str, params_to_optimize
...
def construct_parametric_factor(label, param_equality, params_to_opt, 
    ...
    if status is None:
        status['meaningful'] = False; status['s_and_d_merged'] = True; 
        status['unique_specific_token'] = False; status['unique_token_type'] = False; 
