...
class FourierNN(nn.Module):
    ...
    def forward(self, grid: torch.Tensor) -> torch.Tensor:
        ...
        """
        ann_input = self.model[0](grid)
        V = self.activation(self.linear_v(ann_input))
        U = self.activation(self.linear_u(ann_input))
        for layer in self.model[1:-1]:
            output = self.activation(layer(ann_input))
            ann_input = output * U + (1 - output) * V

        output = self.model[-1](ann_input)
