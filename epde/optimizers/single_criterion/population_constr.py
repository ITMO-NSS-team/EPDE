...
class SystemsPopulationConstructor(object):
    def __init__(self, pool, terms_number : int = 8, max_factors_in_term : int = 2,
        ...
        # ``second_objective`` is accepted (and ignored) because EpdeSearch.fit
        # injects it into the shared ``population_instruct`` dict used by
        # both the single- and multi-objective population constructors;
        # a single-objective front has no second axis, and its objective is
        # chosen by the global single_objective_metric instead (see
        # SoEq.use_default_singleobjective_function). Without this parameter
        # the constructor raised TypeError and single-objective mode could
        # never start.
        self._second_objective = second_objective
