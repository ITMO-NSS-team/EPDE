...
class MOEADDDirector(OptimizationPatternDirector):
    ...
    def use_baseline(self, use_solver: bool = False, use_pic: bool = True,
        ...
        def _solver_free_fitness(second: str = None, metric: str = None):
            ...
            configuration (``globals.discrepancy_metric``, written by
        ...
        if use_solver:
            # ``use_pic`` ONLY selects the second objective (instability in
            # place of the baseline complexity) -- via the same
            # ``resolve_second_objective`` used by the solver-free branch, the
            # SoEq axis registration and the MOEA/D ideal point, so an
            # explicit ``set_second_objective`` override works here too. The
            # backend (and the backend-implied primary discrepancy option) is
            # chosen by ``solver_backend``, independent of ``use_pic``. A
            # complexity second axis needs no filler -- the
            ...
            fitness = _solver_free_fitness(
                global_var.resolve_second_objective(use_pic))
