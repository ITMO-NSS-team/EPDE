...
from epde.src.operators.template import Compound_Operator
...
#        if baseline:
    ...
    def strategy_assembly(self, **kwargs):
        from epde.src.operators.equation_selections import Tournament_selection
        ...
        from epde.src.operators.equation_mutations import PopLevel_mutation, Equation_mutation, Term_mutation, Parameter_mutation
        ...
        from epde.src.operators.equation_crossovers import PopLevel_crossover, Equation_crossover, Param_crossover, Term_crossover
        ...
        from epde.src.operators.equation_sparcity import LASSO_sparsity
        lasso_coeffs = LASSO_sparsity(['sparcity'])
        from epde.src.operators.equation_coeffcalc import LinReg_based_coeffs
        ...
        from epde.src.operators.equation_fitness import L2_fitness
        ...
        from epde.src.operators.equation_right_part_selection import Poplevel_Right_Part_Selector, Eq_Right_Part_Selector
...
class Block(ABC):
    ...
    def op_id(self):
        if not self.id_set:
            self._id =  np.random.randint(0, 1e10)
            self.id_set = True
        return self._id
...
class Evolutionary_block(Block):
    ...
    Arguments:
    -----------
        operator : epde.src.operators.template.Compound_Operator
    ...
    Parameters:
    -----------
        _operator : epde.src.operators.template.Compound_Operator
            The evolutionary operator, contained by the block
