...
# from epde.structure.domain import Domain
...
class PreparedTokens(ABC):
    ...
    def __hash__(self):
        return self._token_family.ftype
...
class ExternalDerivativesTokens(CustomTokens):
    def __init__(self, token_type: str, time_axis: int, base_token_label: list, # token_tensor: np.ndarray,
                 max_orders: Union[int, tuple], params_ranges: dict, # deriv_method: str, deriv_method_kwargs: dict, 
        ...
        self._max_orders = max_orders # deriv_method_kwargs['max_order'] =
        ...
        self.base_name = base_token_label
        self.deriv_names = []; self.deriv_sorted = []
        self.d_ords = max_orders
        for label in base_token_label:
            dnames, _ = define_derivatives(label,
                                           dimensionality=dimensionality,
                                           max_order=max_orders)
            self.deriv_names.extend(dnames); self.deriv_sorted.append(dnames) 
    ...
    def upload(self, trajectory: Dict[str, np.ndarray], cache, preprocesser: PreprocessingPipe, grid: list = [], # deriv_method: str, deriv_method_kwargs: dict,
               traj_id: int = 0, *args, **kwargs) -> None:
        assert set(trajectory.keys()) == set(self.base_name), 'Mismatching keys for trajectories and base name tokens.'

        for t_key, t_vals in trajectory.items():
            # _, derivs_tensor = preprocess_derivatives(t_vals, method=deriv_method,
            #                                           method_kwargs=deriv_method_kwargs)

            # derivs_stacked = prepareVarTensor(t_vals, derivs_tensor, self._time_axis)

            _, derivatives = preprocesser.run(t_vals, grid=grid, max_order=self.d_ords)

            uploadSimpleTokens(self.deriv_names[t_key], cache = cache, tensors = derivatives, subcache_ID = traj_id)
